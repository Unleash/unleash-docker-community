...
const clientID = process.env.UNLEASH_AUTH_CLIENT_ID;
const clientSecret = process.env.UNLEASH_AUTH_CLIENT_SECRET;
const tenantID = process.env.UNLEASH_AUTH_TENANT_ID;

function azureAdminOauth(app, config, services) {
  const { unleashUrl, baseUriPath } = config.server;
    ...
    new OIDCStrategy(
      {
        ...
        clientID,
        clientSecret,
        redirectUrl: `${unleashUrl}/api/auth/callback`,
        responseType: "code",
        responseMode: "query",
        scope: ["openid", "email"],
